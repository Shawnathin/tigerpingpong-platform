...
const MANIFEST_PATH = path.join(REPO_ROOT, "data/media/tables-category-image-map-v1.json");
const RESULTS_DIR = path.join(REPO_ROOT, "exports/tables-category-media");
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "Tables category media upload failed.");
  process.exitCode = 1;
});
...
async function main() {
  ...
  const sourceRoots = {
    downloads: path.resolve(args.downloadsRoot),
    repo: REPO_ROOT
  };
  const verified = [];

  for (const entry of manifest.entries) {
    const sourceRoot = sourceRoots[entry.sourceRoot];
    if (!sourceRoot) {
      throw new Error(`Unknown source root for ${entry.assetId}: ${entry.sourceRoot}`);
    }
    ...
    verified.push({ entry, sourceBuffer, sourcePath });
  }

  const planned = verified.filter(({ entry }) => entry.uploadAction === "upload");

  console.log(
    JSON.stringify(
      {
        mode: args.commit ? "commit" : "dry-run",
        downloadsRoot: sourceRoots.downloads,
        verifiedCount: verified.length,
        uploadCount: planned.length,
        entries: verified.map(({ entry, sourceBuffer }) => ({
          ...
          uploadAction: entry.uploadAction
        }))
      },
      null,
      2
...
function parseArgs(argv) {
  const parsed = {
    commit: false,
    downloadsRoot: path.join(homedir(), "Downloads"),
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--downloads-root" || arg === "--env-file") {
    ...
    if (arg.startsWith("--downloads-root=")) {
      parsed.downloadsRoot = arg.slice("--downloads-root=".length);
      continue;
    }
...
function printHelp() {
  console.log(`Tiger PingPong tables category media uploader

Dry run (default):
  node scripts/media/upload-tables-category-media.mjs

Real upload:
  node scripts/media/upload-tables-category-media.mjs --commit

Options:
  --downloads-root <path>  Approved Downloads folder containing the patio original.
...
async function uploadEntry({ entry, sourceBuffer, sourcePath }, credentials) {
  ...
  const uploadParams = {
    ...
    tags: "tiger-tables-category,storefront,owner-cleared",
