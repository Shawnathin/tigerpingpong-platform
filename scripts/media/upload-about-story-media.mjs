...
const MANIFEST_PATH = path.join(REPO_ROOT, "data/media/about-story-image-map-v1.json");
const RESULTS_DIR = path.join(REPO_ROOT, "exports/about-story-media");
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "About story media upload failed.");
  process.exitCode = 1;
});
...
function printHelp() {
  console.log(`Tiger PingPong About story media uploader

Dry run (default):
  node scripts/media/upload-about-story-media.mjs
...
async function uploadEntry({ entry, sourceBuffer, sourcePath }, credentials) {
  ...
  const uploadParams = {
    ...
    tags: "tiger-about-story,storefront,owner-cleared",
