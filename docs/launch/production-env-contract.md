...
| `SHIPMENT_EMAIL_WEBHOOK_URL`          | API           |                                           Optional | Secret                     | HTTPS URL                                                       | `apps/api/src/config.ts`, `apps/api/src/internal-orders/internal-orders.service.ts`                                                                                                                                                            | Validate only if using shipment handoff endpoint and confirm endpoint receives payload in a test run                                                         | Shipment notification path fails with `Shipment email webhook is not configured` if enabled but unset               | Confirmed by launch docs                                                                                                 |
...
3. Set API variables on `tigerpingpong-platform`:
   - `DATABASE_URL`, `CORS_ORIGIN`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `CHECKOUT_SUCCESS_URL`, `CHECKOUT_CANCEL_URL`, `INTERNAL_ORDERS_API_TOKEN`.
...
- Is shipment email handoff active (`SHIPMENT_EMAIL_WEBHOOK_URL`), or should this remain disabled?
